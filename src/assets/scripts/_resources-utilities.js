...
export function createPagination(dataArray, pageSize, pageInQuery, hrefTemplate) {
	...
	const pagination = {
		...
		hrefs,
		href: {
			next: hrefs[pageInQuery] ?? undefined,
			previous: hrefs[pageInQuery - 2] ?? undefined,
			first: hrefs[0],
			last: hrefs.at(-1),
		},
		pages: dataInChunk,
		page: {
			next: dataInChunk[pageInQuery] ?? undefined,
			previous: dataInChunk[pageInQuery - 2] ?? undefined,
			first: dataInChunk[0],
...
export function renderCheckboxStats(container, checkboxPrefix, checkedValue) {
	for (const value of checkedValue) {
	...
	if (pinnedResources) {
		switch (sortCategory) {
			case 'ascTitle': {
				sortedArray
					.push(
						...pinnedResources.toSorted((a, b) => (a.title.localeCompare(b.title))),
						...unpinnedResources.toSorted((a, b) => (a.title.localeCompare(b.title))),
					);
				break;
			}
			...
			case 'decTitle': {
				sortedArray
					.push(
						...pinnedResources.toSorted((a, b) => (b.title.localeCompare(a.title))),
						...unpinnedResources.toSorted((a, b) => (b.title.localeCompare(a.title))),
					);
				break;
			}
			...
			case 'ascPublishedYear': {
				sortedArray
					.push(
						...pinnedResources.toSorted((a, b) => (a.publishedYear.localeCompare(b.publishedYear))),
						...unpinnedResources.toSorted((a, b) => (a.publishedYear.localeCompare(b.publishedYear))),
					);
				break;
			}
			...
			case 'decPublishedYear': {
				sortedArray
					.push(
						...pinnedResources.toSorted((a, b) => (b.publishedYear.localeCompare(a.publishedYear))),
						...unpinnedResources.toSorted((a, b) => (b.publishedYear.localeCompare(a.publishedYear))),
					);
				break;
			}
			...
			default: {
				sortedArray
					.push(
						...
						...unpinnedResources.toSorted((a, b) => {
							let compare = b.publishedYear.localeCompare(a.publishedYear);
							if (compare === 0) {
								compare = a.title.localeCompare(b.title);
							}
...
export function renderSearchResults(numberOfResources, resourceTopics, resourceTypes) {
	...
	appliedFilterHtml += numberOfResources === 0
		? '<div class=\'resources-no-results\' id=\'resources-result\'><p>Sorry, no results were found based on your applied filters.</p></div>'
		: `<div class='resources-filtered-number' id='resources-result'><p>Showing ${numberOfResources} ${numberOfResources === 1 ? 'result' : 'results'}</p></div>`;
	...
	for (const appliedFilter of appliedFilters) {
		...
		parameters.delete(appliedFilter.name);

		appliedFilterHtml += `<button
			aria-label='Remove the filtering on a resource ${tagType} ${filterLabel}'
			id='filterTag-${appliedFilter.id}'
			class='filter-tag'
			onClick="window.location.href='${globalThis.location.href.split('?')[0]}?${parameters.toString()}'">
...
export function renderResources(resources, resourceTopics, resourceTypes) {
	...
	const hostURL = globalThis.location.host;
	...
	for (const resource of resources) {
		...
		resourcesHtml += `
				...
				<div class='card-link'>
					<a rel='external' href='${resource.link}'>Visit ${escapeSpecialCharactersForHTML(resource.title)}${resourceLink.host === hostURL
						? ''
						: '<svg role=\'presentation\'><use xlink:href=\'#external\' /></svg>'}</a></div>
				</div>
...
export function renderPagination(pagination) {
	...
	for (const [pageNumber, _page] of pagination.pages.entries()) {
		paginationHtml += `
...
export function bindEventListeners() {
	...
	for (const expandButton of expandButtons) {
		...
		expandButton.addEventListener('click', event => {
			...
			const filterType = expandButton.getAttribute('id').split('-')[1];
			expandButton.setAttribute('aria-expanded', expandedState);
			expandButton.setAttribute('aria-label', `${expandedState === 'true' ? 'collapse' : 'expand'} the ${filterType} filter`);
	...
	if (document.querySelector('.reset-button') !== undefined) {
