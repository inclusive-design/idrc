...
export function renderSearchResults(numberOfResources, resourceTopics, resourceTypes) { // eslint-disable-line no-unused-vars
    ...
    if (numberOfResources === 0) {
        appliedFilterHtml += "<div class='resources-no-results' id='resources-sort-result'><p>Sorry, no results were found based on your applied filters.</p></div>";
    } else {
        appliedFilterHtml += `<div class='resources-filtered-number' id='resources-sort-result'><p>Showing ${numberOfResources} ${numberOfResources === 1 ? "result" : "results"}</p></div>`;
    }
